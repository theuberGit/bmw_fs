		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false;// false 로 setting -> 크롬에서도 싱글파일업로드 가능
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/resources/se/sample/photo_uploader/img/btn_confirm2.png");
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/mgmt/gallery/multiplePhotoUpload.do'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/Files/photoUpload',	//샘플 URL입니다.
 			sCallback : '/resources/se/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
